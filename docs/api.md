...
`/api/cards`, `/api/tcgs`, `/api/sets`, `/api/scans`, `/api/collection`, `/api/wishlist`, `/api/decks` — schema and types already exist (see `prisma/schema.prisma`, `packages/types`), routes will be added as each phase lands.
