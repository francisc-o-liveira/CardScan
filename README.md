...
**Prices** come from TCGplayer via [tcgcsv.com](https://tcgcsv.com), a free, keyless daily mirror of TCGplayer's own catalog and price API: `pnpm sync:prices` stores each card's market/low/mid/high price per finish (Normal, Holofoil, Reverse Holofoil …) in USD, plus one market-price history point per day, and the web app shows them on card tiles and card detail (with a 1M/3M/6M/1Y price-history chart). In development the API runs it (and a weekly catalog refresh for every game) by itself in the background — see `AUTO_SYNC` in `.env.example`. The price chart uses TCGplayer's own history, fetched from an unofficial endpoint when a card's chart is opened (once per card and range per day, cached in the database; `TCGPLAYER_HISTORY=false` turns it off). If that endpoint is unavailable the chart falls back to the points our daily sync has recorded. See [`docs/database.md`](docs/database.md#tcgplayer-prices) for coverage per game.
